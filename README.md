...
ダッシュボードから接続するには、ビルド時に `VITE_REALTIME_API_TOKEN` に同じ値を設定して
ください。Vite の `VITE_` 変数はブラウザ向けバンドルに含まれるため、このダッシュボードは
信頼できる利用者だけに公開し、本番環境ではダッシュボード専用の資格情報を用意してください。
トークンが未設定の場合、これらの外部連携機能はフェイルクローズし、クライアントは認証情報
なしで再接続を繰り返しません。
